...
### Data inspection
As an initial step, the imported data structure is checked with `glimpse()`.

```{r}
bike_rides %>%
  glimpse()
```
The output indicates that the data set has `r nrow(bike_rides)` rows, and `r ncol(bike_rides)` columns. The start_station and end_station columns appear to have a significant number of missing --`NA`-- values.
