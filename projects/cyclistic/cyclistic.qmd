...
*Casual-Docked* consistently has the longest median ride duration and appears to capture rides taken on a day pass.
For this analysis I have assumed that *Casual-Docked* rides are primarily visitor traffic.
As visitors are unlikely to take up membership, they can be dropped from the analysis.
...
The changes reduced the casual unlock fee from \$3.30 to \$1.00 and removed 30 minutes included time for classic bikes and e-bike trips.
There is an indication that the median duration for *Casual-Classic* and *Casual-Electric* trips has declined from May 2022 onwards.
...
rides_temp %>%
  ...
  theme_minimal() +
  labs(
    x = "Maximum Temperature <U+00B0>C",
    y = "Rides"
  )
```

```{r}
#| label: fig-avg-wind-daily-rides
#| fig-asp: 1.25
#| dev: "svg"

rides_temp %>%
  ggplot(aes(awnd, n, group = user_bike)) +
  geom_point(alpha = 0.5) +
  geom_smooth(color = "red", alpha = 0.5, method = "lm", se = FALSE) +
  facet_grid(
    rows = vars(user_bike),
    cols = vars(weekend)
  ) +
