...
bike_rides %>%
  group_by(Month = month, Day = day(started_at)) %>%
  summarise("Percent Negative" = mean(ride_time < 0) * 100, .groups = "drop") %>%
...
bike_rides <- bike_rides %>%
  filter(ride_time > 60, ride_time < 86000)
```
...
basic %>%
  ggplot(aes(month, rides, group = user_bike, colour = user_bike)) +
  geom_line() +
  scale_y_continuous(
